...
class UserService {
    ...
    static async checkoutOk(id, total) {
        try {
            ...
            const cartUpdate = await Users.updateOne({ _id: ObjectId(id) }, { $set: { cart: [] } })
            const orderOk = await Orders.create({ order: cartOk.cart })

            let orderUpdate = await Orders.updateOne({ _id: ObjectId(orderOk._id) }, { $set: { userId: id } })
            orderUpdate = await Orders.updateOne({ _id: ObjectId(orderOk._id) }, { $set: { total: total } })
